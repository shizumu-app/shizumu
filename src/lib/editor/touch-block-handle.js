...
// of their own.
//
// block-shell.js builds a chip for every board NodeView that uses it (list,
// blockquote, qaBlock, recipeBlock, codeBlock, chart); table gets its own
// from ShellTableView. Plain paragraphs and headings get none of those —
// on desktop that's fine (the hover pill covers them), but on touch there
// is no hover, and after the chip became the tap target for block actions
// (touch-actions redesign) a phone would otherwise have NO way to reach a
// plain, EMPTY paragraph's insert menu.
//
